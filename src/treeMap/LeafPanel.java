...
public class LeafPanel extends Panel{
	...
	public void draw(ImagePlus resImg) {
		if (hasLabel==true) moveLabel(resImg.getOverlay());
		moveScalebar(resImg);
		...
		for(int imgY=yPos; 		imgY < Math.min(figHeight, yPos+panelHeight); imgY++)
			...
			{
				// if an image is scaled to a smaller size, its pixel array is not updated (cache ;)), 
				// only the sector drawn shrinks. pixels are only re-read if the image becomes larger
				// (disabled atm because of heap space problem when dealing with large images)
				pixels[imgY*figWidth + imgX] = 
						myPanelPixels[(imgY-yPos)*maxW + imgX-xPos]; 
			}
	...
	/** creates a new pixel array, transfers the old data to the center (former: upper left - commented out) and fills the rest with default color */
	private void expandPixelArray()
	{
		...
		for(int y = 0; y<panelHeight; y++)
			for(int x = 0; x<panelWidth; x++)
			{
				if(x>= offSetX && x<offSetX+maxW && y >= offSetY && y < offSetY+maxH)
					myTempPixels[y*panelWidth + x] = myPanelPixels[(y-offSetY)*maxW + (x-offSetX)];
				else
	...
	public void setPixels(ImagePlus image) {
		hasImg = true;
		if (image.getProcessor()==null) image.setProcessor(IJ.getImage().getProcessor());
		System.out.println(image.
				getProcessor() != null);  //TODO is null after opening a new image
	...
	public void setPixels(int[] pixels) {
		...
		else System.out.println("array length does not fit");
	...
	/** returns true if x and y are inside the panel from which a certain tolerance border got subtracted */
	@Override
	...
	private int getXLabelOffset() {
		if(labelPos==LabelPosition.TopLeft ||labelPos==LabelPosition.BottomLeft )
			...
			return this.getX()+this.getW()-xLabelOffset-ip.getStringWidth(labelText);
		}
	}
	...
	private int getYYLabelOffset() {
		if(labelPos==LabelPosition.TopLeft ||labelPos==LabelPosition.TopRight )
			...
			ip.setFont(this.labelFont);
			//ip.setFont(new Font(TextRoi.getFont(), TextRoi.getStyle(), TextRoi.getSize()));
			return this.getY()+this.getH()-yLabelOffset-ip.getFontMetrics().getHeight();
		}
	}
	...
	public void setLabel(ImagePlus resultFigure, String text, int xOffset, int yOffset, LabelPosition pos) {
		...
		labelFont = new Font (TextRoi.getFont(),TextRoi.getStyle(),TextRoi.getSize());
	...
	public void moveLabel(Overlay o) {
		...
		label = new TextRoi(getXLabelOffset(), getYYLabelOffset(), labelText, labelFont);
	...
	public void setScalebar(ImagePlus resultFigure, int xOffset, int yOffset, double width, int height) {
		...
		scaleBarWidth 	= width;
		if (scalebarColor==null) scalebarColor 	= Toolbar.getForegroundColor();
	...
	public void moveScalebar(ImagePlus imp) {
		...
		if(scalebar != null)
			o.remove(scalebar);
		Line l = new Line(this.getX()+this.getW()-xScaleOffset, this.getY()+ this.getH()-yScaleOffset, this.getX()+this.getW()-xScaleOffset-scaleBarWidth+scaleBarHeight, this.getY()+ this.getH()-yScaleOffset);
		...
		if (scaleBarWidth>9) {
			o.add(scalebar);
		//IJ.showStatus (IJ.d2s(scaleBarWidth*this.getImgData().getPixelWidth(),2)+" "+this.getImgData().getUnit());
		imgData.setScalebarLength(scaleBarWidth*this.getImgData().getPixelWidth());
		} else 	{
	...
	public String getScaleBarText() {
		...
		else return "scalebar:"+IJ.d2s(scaleBarWidth*imgData.getPixelWidth(),2)+imgData.getUnit();
	}
