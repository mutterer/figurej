...
// add figure panels and margins as remarkable points
xm = parseInt(Property.get("xmargin"));
ym = parseInt(Property.get("ymargin"));
panels = Property.get("panels");
panels = split (panels,"\n");
for (p=1;p<panels.length;p++) {
  panel = panels[p]; 
  panel = split(panel,",");
  er = newArray( 1*panel[0]+xm, 1*panel[0] + 1*panel[2] / 2, 1*panel[0] + 1*panel[2]-xm, 1*panel[1]+ym, 1*panel[1] + 1*panel[3] / 2, 1*panel[1] + 1*panel[3] -ym); 
  r = Array.concat( r, er ); 
  //Array.print (panel);
}
