...
public abstract class Link extends JFrame {
	...
	protected void store(String tempDir, String fileName) {
		...
		if (leafPanel != null) {
			...
			if (IJ.debugMode) IJ.log (tempDir+fileName.substring(0, fileName.indexOf(".")+1)+"R");
		    File sourceRFile = new File(tempDir+fileName.substring(0, fileName.indexOf(".")+1)+"R");
		    if (sourceRFile.exists())
				try {
					dataSource.setExternalSource(sourceRFile.getCanonicalPath());
				} catch (IOException e) {
			...
			IJ.log("FigureJ error");
			IJ.log("Panel dimensions do not match");
			IJ.log("got "+imp.getWidth()+"x"+imp.getHeight());
			IJ.log("expected "+leafPanel.getW()+"x"+leafPanel.getH());
			IJ.log("-----------------------------");
	...
	protected void updateActivePanel() {
		System.out.println(mainW.toString());
		System.out.println(mainW.getSelectedPanel().toString());
	...
	public static void createFile(String path, String s, boolean allowOverwrite) {
